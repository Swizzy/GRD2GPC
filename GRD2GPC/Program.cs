...
namespace GRD2GPC {
    internal class Program {
        ...
        private static bool _cronusZen;
        ...
        private static readonly string[] ZenButtonNames = {
                                                                "PS4_PS",
                                                                "PS4_SHARE",
                                                                "PS4_OPTIONS",
                                                                "PS4_R1",
        ...
        private static void Main(string[] args) {
            ...
            using (var fs = File.OpenRead(grdname)) {
                while (true) {
                    var r = Record.FromStream(fs, _cronusZen);
                    if (r != null)
                        records.Add(r);
                    else
            ...
            using (var fs = File.OpenWrite(gpcname)) {
                using (var sw = new StreamWriter(fs)) {
                    ...
                    var sval = GenerateSetVal(records[0], new Record(_cronusZen));
                    if (!string.IsNullOrWhiteSpace(sval))
                        sw.WriteLine(sval.TrimEnd());
                    long waitTime = 0;
                    for (var i = 1; i < records.Count; i++) {
                        sval = GenerateSetVal(records[i], records[i - 1]);
                        if (!string.IsNullOrWhiteSpace(sval) || (waitTime >= 4000 && _cronusZen == false)) {
                            ...
                            if (waitTime > 4000 && _cronusZen == false)
                                waitTime = 4000;
        ...
        private static string GetSetVal(int index, int value)
        {
            if (value == 0)
                return null;
            else
            {
                if (_cronusZen)
                    return string.Format("\tset_val({0}, {1});", ZenButtonNames[index], value);
                else
                    return string.Format("\tset_val({0}, {1});", ButtonNames[index], value);
            }
        }
        ...
        private static string GenerateSetVal(Record data, Record last, bool overrideChanged = false) {
            ...
            if (_cronusZen)
            {
                inp = GetSetVal(30, GetNewValue(data.Buttons[30], last.Buttons[30], out changed));
                if (inp != null && (changed || overrideChanged))
                    sb.AppendLine(inp);
                inp = GetSetVal(31, data.Buttons[31]);
                if (inp != null && data.Buttons[31] != 0)
                    sb.AppendLine(inp);
                inp = GetSetVal(32, data.Buttons[32]);
                if (inp != null && data.Buttons[32] != 0)
                    sb.AppendLine(inp);
                inp = GetSetVal(33, GetNewValue(data.Buttons[33], last.Buttons[33], out changed));
                if (inp != null && (changed || overrideChanged))
                    sb.AppendLine(inp);
            }
            return sb.ToString();
